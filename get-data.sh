#!/bin/bash

# TODO instead of writing to a file, use sockets to communicate directly with server
# TODO make smart regex into 2 functions to avoid repetition

echo -n > data/disks

disk_blks=$(lsblk -o NAME,TYPE | awk '$2=="disk" {print $1}')
for i in $disk_blks;
do
    smart=$(smartctl -iA /dev/$i)
    model_family=$(echo "$smart" | grep "Model Family:" | sed 's/^Model Family:\s*\(\S.*\)$/\1/g')
    serial_number=$(echo "$smart" | grep "Serial Number:" | sed 's/^Serial Number:\s*\(\S.*\)$/\1/g')
    rotation_rate=$(echo "$smart" | grep "Rotation Rate:" | sed 's/^Rotation Rate:\s*\(\S.*\)$/\1/g')

    smart_attributes=$(echo "$smart" | awk '/ID#\s*ATTRIBUTE_NAME\s*/{flag=1} /SMART Error Log Version:/{flag=0} flag')

    power_on_hours=$(echo "$smart_attributes" | awk '$2=="Power_On_Hours" {print $10}')
    power_cycle_count=$(echo "$smart_attributes" | awk '$2=="Power_Cycle_Count" {print $10}')
    raw_read_error_rate=$(echo "$smart_attributes" | awk '$2=="Raw_Read_Error_Rate" {print $10}')
    temperature=$(echo "$smart_attributes" | awk '$2=="Temperature_Celsius" {print $10}')
   
    echo "/dev/"$i",$model_family,$serial_number,$rotation_rate,$power_on_hours,$power_cycle_count,$raw_read_error_rate,$temperature" >> data/disks
done

parts_info=$(lsblk -o TYPE,FSTYPE,FSUSED,FSSIZE,MOUNTPOINT,NAME,NAME -b | awk '$1=="part" {print "/dev/"$7","$5","$2","$3","$4}')
echo "$parts_info" > data/parts


